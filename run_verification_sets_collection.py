...
def collect_mean_verification_set_stats():
    ...
    for json_file_path in file_paths:
        ...
        X_test_noisy_random, y_test_noisy_random = get_fingerprinted_random_points_noisy_data()
        # X_test_fingerprinted_noisy, y_test_fingerprinted_noisy = get_fingerprinted_data_noisy_as_verification_set()
        X_test_fingerprinted_calm, y_test_fingerprinted_calm = get_fingerprinted_data_as_verification_set()

        mae_calm_random, mse_calm_random, rmse_calm_random, medae_calm_random, min_error_calm_random, max_error_calm_random, predicted_locations_calm_random, absolute_errors_calm_random = get_ann_stats(
            ann_weights, X_test_calm_random, y_test_calm_random)
        mae_noisy_random, mse_noisy_random, rmse_noisy_random, medae_noisy_random, min_error_noisy_random, max_error_noisy_random, predicted_locations_noisy_random, absolute_errors_noisy_random = get_ann_stats(
            ann_weights, X_test_noisy_random, y_test_noisy_random)
        # mae_fingerprinted_noisy, mse_fingerprinted_noisy, rmse_fingerprinted_noisy, medae_fingerprinted_noisy, min_error_fingerprinted_noisy, max_error_fingerprinted_noisy, predicted_locations_fingerprinted_noisy, absolute_errors_fingerprinted_noisy = get_ann_stats(
        ...
        mea_mean = np.mean(
            [mae_calm_random, mae_noisy_random, mae_fingerprinted_calm])
        mse_mean = np.mean(
            [mse_calm_random, mse_noisy_random, mse_fingerprinted_calm])
        rmse_mean = np.mean(
            [rmse_calm_random, rmse_noisy_random, rmse_fingerprinted_calm])
        medae_mean = np.mean(
            [medae_calm_random, medae_noisy_random, medae_fingerprinted_calm])
        min_error_mean = np.mean(
            [min_error_calm_random, min_error_noisy_random, min_error_fingerprinted_calm])
        max_error_mean = np.mean(
            [max_error_calm_random, max_error_noisy_random, max_error_fingerprinted_calm])
    ...
    subfolder = "verification_stats/mean_value_noisy_models"
    if not os.path.exists(subfolder):
        os.mkdir(subfolder)
...
def collect_mean_verification_set_stats_from_random_search():
        ...
        if filename.endswith(".json"):
            with open(file_path, 'r') as file:
                try:
                    ...
                    min_error_mean = np.mean(
                        [min_error_calm_random, min_error_noisy_random, min_error_fingerprinted_noisy])
                    max_error_mean = np.mean(
                        [max_error_calm_random, max_error_noisy_random, max_error_fingerprinted_noisy])
                    ...
                    json_data.append({
                        ...
                        "Min Error_Mean": min_error_mean,
                        "Max Error_Mean": max_error_mean,
                    })
                except json.JSONDecodeError:
                    print(f"Error reading JSON file: {file_path}")
    ...
    subfolder = "verification_stats/random_distribution_runs_gbest/with_velocity_bounds/mean_values"
